...
const JobsRepository = {
  async create(job) {
    ...
    await run(
      `INSERT INTO jobs (id, name, video_asset_id, destination_id, preset_id, loop_enabled, crossfade_seconds, status, invalid_reason, enabled, auto_recovery_enabled, audio_replace_config, hot_swap_mode, scenes_json, swap_rules_json)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        job.name,
        ...
        job.enabled === undefined ? 1 : job.enabled ? 1 : 0,
        job.auto_recovery_enabled ? 1 : 0,
        job.audio_replace_config || null,
        job.hot_swap_mode || null,
        job.scenes_json || null,
        job.swap_rules_json || null,
      ]
    );
    return { ...job, id };
  },
  async update(id, updates) {
    ...
    const merged = {
      ...
      enabled: updates.enabled === undefined ? job.enabled : updates.enabled ? 1 : 0,
      auto_recovery_enabled:
        updates.auto_recovery_enabled === undefined ? job.auto_recovery_enabled : updates.auto_recovery_enabled ? 1 : 0,
      audio_replace_config:
        updates.audio_replace_config === undefined ? job.audio_replace_config : updates.audio_replace_config,
      hot_swap_mode: updates.hot_swap_mode === undefined ? job.hot_swap_mode : updates.hot_swap_mode,
      scenes_json: updates.scenes_json === undefined ? job.scenes_json : updates.scenes_json,
      swap_rules_json: updates.swap_rules_json === undefined ? job.swap_rules_json : updates.swap_rules_json,
    };
    await run(
      `UPDATE jobs
       SET name = ?, video_asset_id = ?, destination_id = ?, preset_id = ?, loop_enabled = ?, crossfade_seconds = ?, invalid_reason = ?, enabled = ?, auto_recovery_enabled = ?, audio_replace_config = ?, hot_swap_mode = ?, scenes_json = ?, swap_rules_json = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [
        merged.name,
        ...
        merged.enabled,
        merged.auto_recovery_enabled,
        merged.audio_replace_config || null,
        merged.hot_swap_mode || null,
        merged.scenes_json || null,
        merged.swap_rules_json || null,
        id,
      ]
    );
    return this.findById(id);
  ...
  list(filter = {}) {
    const clauses = [];
    const params = [];
    if (filter.fix_required) {
      clauses.push('(status = "invalid" OR enabled = 0)');
    }
    const where = clauses.length ? `WHERE ${clauses.join(' AND ')}` : '';
    return all(`SELECT * FROM jobs ${where} ORDER BY created_at DESC`, params);
  },
  findByAsset(assetId) {
    return all('SELECT * FROM jobs WHERE video_asset_id = ?', [assetId]);
  },
  findByDestination(destinationId) {
    return all('SELECT * FROM jobs WHERE destination_id = ?', [destinationId]);
  },
  findByPreset(presetId) {
    return all('SELECT * FROM jobs WHERE preset_id = ?', [presetId]);
  },
  markInvalid(id, reason) {
    return run(
      'UPDATE jobs SET status = "invalid", invalid_reason = ?, enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [reason, id]
    );
  },
  markEnabled(id) {
    return run(
      'UPDATE jobs SET enabled = 1, invalid_reason = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [id]
    );
  },
}; 
...
const EventsRepository = {
  ...
  deleteOlderThan(cutoffTimestamp, excludeSessionIds = []) {
    let sql = 'DELETE FROM events WHERE created_at < ?';
    const params = [cutoffTimestamp];
    if (excludeSessionIds.length) {
      const placeholders = excludeSessionIds.map(() => '?').join(',');
      sql += ` AND (session_id IS NULL OR session_id NOT IN (${placeholders}))`;
...
const LogPartsRepository = {
  ...
  deleteOlderThan(cutoffTimestamp, excludeSessionIds = []) {
    let sql = 'DELETE FROM log_parts WHERE created_at < ?';
    const params = [cutoffTimestamp];
    if (excludeSessionIds.length) {
      const placeholders = excludeSessionIds.map(() => '?').join(',');
      sql += ` AND (session_id IS NULL OR session_id NOT IN (${placeholders}))`;
...
const SettingsRepository = {
  ...
  async write(settings) {
    await run(
      `INSERT OR IGNORE INTO settings (id, timezone, language, retention_days, keep_forever, safety_cap_enabled, benchmark_profile, telegram_enabled, telegram_bot_token_enc, telegram_chat_id, telegram_events_json, license_tier)
       VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        settings.timezone,
        settings.language,
    ...
    await run(
      `UPDATE settings SET timezone = ?, language = ?, retention_days = ?, keep_forever = ?, safety_cap_enabled = ?, benchmark_profile = ?, telegram_enabled = ?, telegram_bot_token_enc = ?, telegram_chat_id = ?, telegram_events_json = ?, license_tier = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1`,
      [
        settings.timezone,
        settings.language,
        settings.retention_days,
