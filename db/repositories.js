...
const JobsRepository = {
  async create(job) {
    ...
    await run(
      `INSERT INTO jobs (id, name, video_asset_id, destination_id, preset_id, loop_enabled, crossfade_seconds, status, invalid_reason, enabled)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        job.name,
  ...
  async update(id, updates) {
    ...
    await run(
      `UPDATE jobs
       SET name = ?, video_asset_id = ?, destination_id = ?, preset_id = ?, loop_enabled = ?, crossfade_seconds = ?, invalid_reason = ?, enabled = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
      [
        merged.name,
...
const SettingsRepository = {
  ...
  async write(settings) {
    await run(
      `INSERT OR IGNORE INTO settings (id, timezone, language, retention_days, keep_forever, safety_cap_enabled, benchmark_profile, telegram_enabled, telegram_bot_token_enc, telegram_chat_id, telegram_events_json)
       VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        settings.timezone,
        settings.language,
    ...
    await run(
      `UPDATE settings SET timezone = ?, language = ?, retention_days = ?, keep_forever = ?, safety_cap_enabled = ?, benchmark_profile = ?, telegram_enabled = ?, telegram_bot_token_enc = ?, telegram_chat_id = ?, telegram_events_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = 1`,
      [
        settings.timezone,
        settings.language,
        settings.retention_days,
