...
- **Uploads and /data Usage**: Upload middleware in `middleware/uploadMiddleware.js` stores videos and avatars on disk using paths from `utils/storage.js`. `utils/storage.js` points storage to `/data/assets/videos`, `/data/assets/audios`, `/data/assets/sfx`, `/data/assets/avatars`, `/data/assets/thumbs`, and `/data/logs`, creating directories at startup via `ensureDirectories()` in `app.js`. Static serving for uploads uses these paths via `/uploads/...` routes.
...
2. Implement asset ingestion: upload endpoint with 500MB cap, ffprobe metadata + thumbnail saved to `/data/assets/*` and `/data/assets/thumbs`.
