...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/bangtutorial/zenstream/main/scripts/install.sh)
```
...
- Place ZenStream under `/opt/zenstream` by default and write a `.env` with `PORT=6969`, `DATA_DIR=/data`, and a generated secret (existing `.env` is left untouched).
- Start the stack with `docker compose up -d --build`.

Then open `http://<ip>:6969` to finish the setup wizard.

> Secrets: set `INSTALL_SECRET` (or reuse `SESSION_SECRET`) to encrypt stored stream keys. Both values live in `.env`; the installer seeds both with the same random value. If `SESSION_SECRET` is not set, ZenStream will generate one on first boot and persist it at `/data/config/session_secret` so sessions stay valid across restarts.
