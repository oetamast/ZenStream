...
> Secrets: set `INSTALL_SECRET` (or reuse `SESSION_SECRET`) to encrypt stored stream keys. Both values live in `.env`; the installer seeds both with the same random value. If `SESSION_SECRET` is not set, ZenStream will persist one at `/data/config/session_secret` so sessions stay valid across restarts.
